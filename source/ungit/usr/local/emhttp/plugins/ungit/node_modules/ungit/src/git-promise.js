...
var gitQueue = async.queue(function (args, callback) {
  ...
  var stdout = '';
  var stderr = '';
  var gitProcess = child_process.spawn('git', args.commands, {
    cwd: args.repoPath,
...
var gitExecutorProm = function gitExecutorProm(args, retryCount) {
 ...
 */
var git = function git(commands, repoPath, allowError, outPipe, inPipe, timeout) {
  var args = {};
...
var getGitError = function getGitError(args, stderr, stdout) {
  var err = {};
  err.isGitError = true;
  ...
  } else if (err.stderr.indexOf('unable to access') != -1 && err.stderr.indexOf('Could not resolve host:') != -1 || err.stderr.indexOf('Could not resolve hostname') != -1) {
...
git.status = function (repoPath, file) {
  return Bluebird.props({
    numStatsStaged: git(['diff', '--no-renames', '--numstat', '--cached', '--', file || ''], repoPath).then(gitParser.parseGitStatusNumstat),
    numStatsUnstaged: git(['diff', '--no-renames', '--numstat', '--', file || ''], repoPath).then(gitParser.parseGitStatusNumstat),
    status: git(['status', '-s', '-b', '-u', file || ''], repoPath).then(gitParser.parseGitStatus).then(function (status) {
      return Bluebird.props({
        ...
        if (status.inMerge || status.inCherry) {
          return fs.readFileAsync(path.join(repoPath, '.git', 'MERGE_MSG'), { encoding: 'utf8' }).then(function (commitMessage) {
...
git.getRemoteAddress = function (repoPath, remoteName) {
  return git(['config', '--get', 'remote.' + remoteName + '.url'], repoPath).then(function (text) {
...
git.stashExecuteAndPop = function (commands, repoPath, allowError, outPipe, inPipe, timeout) {
  ...
  return git(['stash'], repoPath).catch(function (err) {
...
git.diffFile = function (repoPath, filename, sha1) {
  ...
  return git.revParse(repoPath).then(function (revParse) {
  ...
  .then(function (status) {
    ...
    if (!file && !sha1) {
      return fs.isExists(path.join(repoPath, filename)).then(function (isExist) {
        if (isExist) return [];else throw { error: 'No such file: ' + filename, errorCode: 'no-such-file' };
      });
      ...
      return exec.catch(function (err) {
        ...
        if (sha1 && err && err.error.indexOf('bad revision') > -1) return git(newFileDiffArgs, repoPath, true);
      });
    }
  });
};
...
git.getCurrentBranch = function (repoPath) {
  return git.revParse(repoPath).then(function (revResult) {
    var HEADFile = path.join(revResult.gitRootPath, '.git', 'HEAD');
...
git.discardAllChanges = function (repoPath) {
  return git(['reset', '--hard', 'HEAD'], repoPath).then(function () {
    return git(['clean', '-fd'], repoPath);
  });
};

git.discardChangesInFile = function (repoPath, filename) {
  return git.status(repoPath, filename).then(function (status) {
    ...
    if (!fileStatus.staged) {
      ...
      if (fileStatus.isNew) {
        return fs.unlinkAsync(path.join(repoPath, filename)).catch(function (err) {
  ...
  return new Bluebird(function (resolve, reject) {
    ...
    for (var v in files) {
      ...
      if (fileStatus.removed) {
        ...
        diffPatchPromises.push(git(['diff', file.name.trim()], repoPath).then(gitParser.parsePatchDiffResult.bind(null, file.patchLineList)).then(git.applyPatchedDiff.bind(null, repoPath)));
      } else {
        toAdd.push(file.name.trim());
      }
    }

    var commitPromiseChain = Bluebird.resolve().then(function () {
...
git.revParse = function (repoPath) {
  return git(['rev-parse', '--is-inside-work-tree', '--is-bare-repository', '--show-toplevel'], repoPath).then(function (result) {
    var resultLines = result.toString().split('\n');
    var rootPath = path.normalize(resultLines[2] ? resultLines[2] : repoPath);
