...
exports.registerApi = function (env) {
      ...
      socket.on('watch', function (data, callback) {
        ...
        try {
          socket.watcher = fs.watch(data.path, function (event, filename) {
            ...
            if (!filename || filename != '.git' && filename.indexOf('.git/') != 0) workingTreeChanged();
          });
  ...
  var emitWorkingTreeChanged = function emitWorkingTreeChanged(repoPath) {
    if (io) {
  ...
  var emitGitDirectoryChanged = function emitGitDirectoryChanged(repoPath) {
    if (io) {
  ...
  var credentialsOption = function credentialsOption(socketId) {
  ...
  app.post(exports.pathPrefix + '/fetch', ensureAuthenticated, ensurePathExists, ensureValidSocketId, function (req, res) {
    ...
    var task = gitPromise({
      commands: credentialsOption(req.body.socketId).concat(['fetch', req.body.remote, req.body.ref ? req.body.ref : '', config.autoPruneOnFetch ? '--prune' : '']),
      repoPath: req.body.path,
      timeout: timeoutMs
    });
  ...
  app.post(exports.pathPrefix + '/push', ensureAuthenticated, ensurePathExists, ensureValidSocketId, function (req, res) {
    ...
    if (res.setTimeout) res.setTimeout(timeoutMs);
    var task = gitPromise({
      commands: credentialsOption(req.body.socketId).concat(['push', req.body.remote, (req.body.refSpec ? req.body.refSpec : 'HEAD') + (req.body.remoteBranch ? ':' + req.body.remoteBranch : ''), req.body.force ? '-f' : '']),
      repoPath: req.body.path,
      timeout: timeoutMs
    });
  ...
  app.post(exports.pathPrefix + '/commit', ensureAuthenticated, ensurePathExists, function (req, res) {
    jsonResultOrFailProm(res, gitPromise.commit(req.body.path, req.body.amend, req.body.message, req.body.files)).then(emitGitDirectoryChanged.bind(null, req.body.path)).then(emitWorkingTreeChanged.bind(null, req.body.path));
  });
  ...
  app.get(exports.pathPrefix + '/log', ensureAuthenticated, ensurePathExists, function (req, res) {
    ...
    var task = gitPromise(['log', '--decorate=full', '--date=default', '--pretty=fuller', '--branches', '--tags', '--remotes', '--parents', '--no-notes', '--numstat', '--date-order', limit], req.query.path).then(gitParser.parseGitLog).catch(function (err) {
      if (err.stderr.indexOf('fatal: bad default revision \'HEAD\'') == 0) return [];else if (/fatal: your current branch \'.+\' does not have any commits yet.*/.test(err.stderr)) return [];else if (err.stderr.indexOf('fatal: Not a git repository') == 0) return [];else throw err;
    });
  ...
  app.get(exports.pathPrefix + '/remote/tags', ensureAuthenticated, ensurePathExists, ensureValidSocketId, function (req, res) {
    var task = gitPromise(credentialsOption(req.query.socketId).concat(['ls-remote', '--tags', req.query.remote]), req.query.path).then(gitParser.parseGitLsRemote).then(function (result) {
  ...
  app.delete(exports.pathPrefix + '/remote/tags', ensureAuthenticated, ensurePathExists, function (req, res) {
    ...
    jsonResultOrFailProm(res, gitPromise(commands, req.query.path)).finally(emitGitDirectoryChanged.bind(null, req.query.path));
  });
  ...
  app.get(exports.pathPrefix + '/stashes', ensureAuthenticated, ensurePathExists, function (req, res) {
    var task = gitPromise(['stash', 'list', '--decorate=full', '--pretty=fuller'], req.query.path).then(gitParser.parseGitLog).then(function (items) {
